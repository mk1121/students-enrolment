...
enrollmentSchema.virtual('duration').get(function getDuration() {
  if (this.startDate && this.completionDate) {
    return Math.ceil(
      (this.completionDate - this.startDate) / (1000 * 60 * 60 * 24)
    );
  }
  return null;
});
...
enrollmentSchema.virtual('isCompleted').get(function getIsCompleted() {
  return this.status === 'completed';
});

// Virtual for isActive
enrollmentSchema
  .virtual('isEnrollmentActive')
  .get(function getIsEnrollmentActive() {
    return this.status === 'active' && this.isActive;
  });

// Method to update progress
enrollmentSchema.methods.updateProgress = function updateProgress(
  completedLessons,
  totalLessons
) {
  if (totalLessons > 0) {
...
enrollmentSchema.methods.addCompletedLesson = function addCompletedLesson(
  lessonId
) {
...
enrollmentSchema.methods.issueCertificate = function issueCertificate() {
...
enrollmentSchema.pre('save', function preSaveUpdateLastAccessed(next) {
