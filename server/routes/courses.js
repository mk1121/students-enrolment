...
router.get(
  ...
  [
    ...
    query('sortBy')
      .optional()
      .isIn(['title', 'price', 'rating', 'createdAt'])
...
router.get(
  '/:id',
  [optionalAuth, param('id').isMongoId().withMessage('Invalid course ID')],
  async (req, res) => {
    try {
      const errors = validationResult(req);
      if (!errors.isEmpty()) {
    ...
    try {
      ...
      if (
        ...
        return res.status(403).json({
          message:
            'Access denied. Only course instructor can update this course.',
        });
      }
  ...
  async (req, res) => {
    try {
      ...
      if (
        ...
        return res.status(403).json({
          message:
            'Access denied. Only course instructor can delete this course.',
        });
      }
